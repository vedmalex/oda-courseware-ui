...
export default ({ queries, resources }) => ({
  query: params => queries.Following.GET_MANY_REFERENCE[params.target],
  parseResponse: (response, params) => {
    const data = reshape(queries.Following.GET_MANY_REFERENCE_RESULT[params.target], response.data);
    return {
